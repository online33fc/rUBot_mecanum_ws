...
class rUBot:
    ...
    def callbackLaser(self, scan):
        ...
        if closestDistance < self._distanceLaser:
            if -20 < angleClosestDistance < 20:
                self._msg.linear.x = self._backwardSpeed * self._speedFactor  # backward
            elif 20 <= angleClosestDistance < 160:
                self._msg.linear.y = -self._lateralSpeed * self._speedFactor  # left
            elif -160 < angleClosestDistance <= -20:
                self._msg.linear.y = self._lateralSpeed * self._speedFactor  # right
            else:
